...
import {
  ...
  Activity, AlertTriangle, MessageSquare, Image as ImageIcon, FileText,
} from "lucide-react";
...
export default function Admin() {
  ...
  async function loadInsights(t: string) {
    setInsightsLoading(true);
    try {
      const res = await fetch("/api/owner/assistant/overview", { headers: { Authorization: `Bearer ${t}` } });
      if (!res.ok) return;
      const data = await res.json();
  ...
  useEffect(() => {
    if (token) { loadUsers(token); loadStats(token); loadInsights(token); }
  }, [token]);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground" dir="rtl">
      ...
      <header className="border-b border-border bg-black/90 backdrop-blur px-4 sm:px-6 py-4 flex items-center justify-between sticky top-0 z-40">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={() => { if (token) { loadUsers(token); loadStats(token); loadInsights(token); } }}
      ...
      <main className="max-w-6xl mx-auto px-4 py-8 space-y-8">
        ...
        {/* Hamzawi — Owner Assistant (read-only, data-driven report) */}
